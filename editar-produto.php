<?php 
    ...
    if (isset($_POST['editar'])){
        // Objeto do tipo produto com os campos do form e conversão de preço formatado
        $produto = new Produto(
            (int) $_POST['id'], 
            $_POST['tipo'], 
            $_POST['nome'], 
            $_POST['descricao'], 
            (float) str_replace(['R$ ', '.', ','], ['', '', '.'], $_POST['preco'])
        );
...
<head>
    ...
    <title>Blackout - Editar Produto</title>
</head>
<body class="bg-neutral-950 text-neutral-200 font-sans selection:bg-neutral-800 antialiased min-h-screen flex flex-col justify-between relative overflow-x-hidden">
    ...
    <main class="flex-grow flex items-center justify-center px-6 py-12 relative z-10">
        <div class="w-full max-w-lg bg-neutral-900/40 border border-neutral-900/85 rounded-3xl p-8 backdrop-blur-xl shadow-2xl">
            ...
            <div class="flex flex-col items-center text-center mb-8">
                ...
                <h1 class="text-2xl font-extrabold text-white tracking-tight">EDITAR PRODUTO</h1>
                <p class="text-xs text-neutral-500 mt-1">Altere as informações necessárias do item selecionado</p>
            </div>

            <!-- Form -->
            <form method="post" enctype="multipart/form-data" class="space-y-6">

                <!-- Seletor Dinâmico de Categoria (Café / Almoço) -->
                <div class="space-y-2">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="relative block cursor-pointer select-none">
                            <input type="radio" id="cafe" name="tipo" value="Café" <?= $produto->getTipo() == "Café" ? "checked" : "" ?> class="peer sr-only">
                        ...
                        <label class="relative block cursor-pointer select-none">
                            <input type="radio" id="almoco" name="tipo" value="Almoço" <?= $produto->getTipo() == "Almoço" ? "checked" : "" ?> class="peer sr-only">
                            <div class="flex items-center justify-center gap-2 px-4 py-3.5 rounded-2xl bg-black/60 border border-neutral-900 text-sm text-neutral-400 peer-checked:border-amber-500 peer-checked:bg-amber-500/10 peer-checked:text-amber-400 hover:border-neutral-800 transition-all font-medium">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input 
                            ...
                            name="preco" 
                            placeholder="0,00" 
                            value="<?= number_format($produto->getPreco(), 2, ',', '.') ?>"
                            required 
                ...
                <!-- Preview de Imagem Atual (Se já possuir uma imagem vinculada) -->
                <?php if ($produto->getImagem()): ?>
                    <div class="space-y-2">
                        <label class="text-xs font-semibold uppercase tracking-wider text-neutral-400 block">Foto Atual Cadastrada</label>
                        <div class="flex items-center gap-4 p-4 rounded-2xl bg-black/50 border border-neutral-900">
                            <img src="<?= $produto->getImagemDiretorio() ?>" class="w-20 h-20 rounded-xl object-cover border border-neutral-800" alt="Imagem atual do produto">
                            <div class="overflow-hidden">
                                <p class="text-xs font-bold text-white truncate"><?= basename($produto->getImagem()) ?></p>
                                <p class="text-[10px] text-neutral-500 mt-1">Caso queira trocar de imagem, envie um novo arquivo abaixo.</p>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
                ...
                <div class="space-y-2">
                    <label class="text-xs font-semibold uppercase tracking-wider text-neutral-400 block">Substituir Foto (Opcional)</label>
                    <label for="imagem" class="group flex flex-col items-center justify-center gap-3 px-6 py-6 border border-dashed border-neutral-800 rounded-2xl bg-black/40 hover:border-amber-500/40 cursor-pointer transition-all">
                        ...
                        <div class="text-xs text-neutral-400 text-center" id="upload-text">
                            <span class="font-semibold text-amber-500 group-hover:text-amber-400">Clique para selecionar</span> uma nova foto
                        </div>
                ...
                <!-- Campo Hidden com ID do Produto -->
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery-maskmoney/3.0.2/jquery.maskMoney.min.js" xintegrity="sha512-Rdk63VC+1UYzGSgd3u2iadi0joUrcwX0IWp2rTh6KXFoAmgOjRS99Vynz1lJPT8dLjvo6JZOqpAHJyfCEZ5KoA==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            fileInput.addEventListener('change', (e) => {
                if(e.target.files.length > 0) {
                    ...
                    uploadText.innerHTML = `Novo arquivo: <span class="font-semibold text-emerald-400">${fileName}</span>`;
                    uploadLimits.textContent = "Excelente! Clique no botão salvar para aplicar as alterações.";
